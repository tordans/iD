...
describe('collectOsmEntityIds', () => {
  ...
  it('parses MapRoulette title forms', () => {
  ...
  it('parses long-form way/node/relation ids', () => {
  ...
  it('collects ids from nested objects via preferred identity keys', () => {
    const task = {
      title: 'w100@0',
      name: 'node/200',
      osmId: 'r300',
  ...
  it('walks nested arrays and trims string values', () => {
    const payload = [
      '  w111@0  ',
  ...
  it('skips coordinate blobs but still reads feature properties and titles', () => {
    const feature = {
      geometry: {
        type: 'Point',
        coordinates: [0, 0],
  ...
  it('stops walking beyond the depth limit', () => {
    let deep: unknown = 'w123';
    for (let i = 0; i < 6; i++) {
      deep = { nested: deep };
    }
    expect(collectOsmEntityIds(deep)).toEqual([]);

    let reachable: unknown = 'w123';
    for (let i = 0; i < 5; i++) {
  ...
  it('collects from mixed MapRoulette-like sources in one call', () => {
    const task = {
      id: 987654,
      title: 'w42@0',
  ...
  it('reads @id from task.geometries features (V4)', () => {
    const task = {
      title: 'unrelated',
      geometries: {
  ...
  it('infers node/way from numeric osmid + geometry type (V4)', () => {
  ...
  it('merges title, V4 @id, and body text without duplicates', () => {
    const task = {
      title: 'w1@0',
  ...
  it('does not invent ids from bare numeric properties.id', () => {
    expect(collectOsmEntityIds({
      type: 'Feature',
      properties: { id: 123 },
...
describe('parseOsmEntityIdFromFeatureProperties', () => {
  it('parses typed @id / id / osm_id strings', () => {
  ...
  it('uses explicit @type / osm_type with numeric ids', () => {
  ...
  it('rejects short forms and bare numeric id for the V4 path', () => {
...
describe('collectOsmEntityIdsFromGeometries', () => {
  it('collects from every feature, not only the first', () => {
